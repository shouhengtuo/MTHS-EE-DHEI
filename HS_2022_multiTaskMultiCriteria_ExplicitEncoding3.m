 function [Task,NC,flag,DFes] = HS_2022_multiTaskMultiCriteria_ExplicitEncoding3(data,dim_epi,HMS,max_iter,CX,objFunction,K0,K,F)
...
n=size(data,2);
State=data(:,n);
% K = dim_epi - 1; % 任务数 2-->epi_dim ，max taskNum ( from 2 to K )
...
for k = 1:taskNum
     ki = ki + 1;
    %% 初始化 initialize 
   dim = Orders(ki); % 第ki个任务的维度(阶数）为k+1, the dimensionality of current task is dim 
    X = zeros(HMS,dim);
    HM = X;
    snp=[];
    for i=1:HMS
        snp(1)=ceil(rand*SNPs);        
        for j=2:dim % 检查每个个体中是否有重复的SNP 位点, check whether there are duplicate SNPs in ith SNP combination.
        ...
        X(i,:)=snp2;   %% X中存放有序的解, the sorted SNP combination        
        HM(i,:)=temp;  %% HM中相应存放无序解, the unsorted SNP combination      
        Scores(i,:) = objFunction(data(:,HM(i,:)),State); %% calculate the fitness values
        snp=[];
    end
    %% for each solution, there are CriteriaNum fitness values that are calculated by K2-score,JS-score,ME-score and JE_Score
    for j = 1:CriteriaNum  
        Criteria(j).X = X;
        Criteria(j).HM = HM;
   ...
   for k = 1 : taskNum
       ...
       if rand < TP %transfer == 1 
           ...
           Flag = 1;          
               ...
               if k > kr %% 从阶数低的任务 向 阶数高的任务 整体迁移, transfer knowledge from kr-task to k task
                     a = ceil(rand*HMS);
                     ...
                     Xtemp = Xnew;
                     Xnew = sort(Xnew);
               else  %% 从阶数高的任务选择迁移   transfer knowledge from kr-task to k task  (kr>k)
           ...
           if rand < TP2 
 		    %% 从相同评价标准的 记忆库中组合学习   generate a new solution from four HMs of current task
               CJ = ceil(rand*CriteriaNum);
             %% ---------------------------------------------- 
              i=1;
              while i<= dim
        ...
        %                                c = ceil(rand*CriteriaNum);
		   ...
		   %% 从不同标准的记忆库中组合优化 generate a new solution from HMs of other tasks
                %% ----------------------------------------------
              i=1;
              while i<= dim
                      if rand<HMCR  
