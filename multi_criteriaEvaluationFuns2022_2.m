% function [K2Score,Gtest,LR,Gini_Score,JE_Score,ME] = multi_criteriaEvaluationFuns(snp_com,state)
 function Scores = multi_criteriaEvaluationFuns2022_2(snp_com,state)
...
B = abs((disease + 0.1)./ (0.1+ control) ).^0.2;
BP = B./sum(B) + 1e-6;
...
%    WS = 1 / sum((Pdisease-Pcontrol).^2);
 ...
 Scores = [K2Score,JS,ME,JE_Score];
%   Scores = [K2Score,HBP,LR,JE_Score];  
%Scores = [JS,LR,ME,JE_Score];  
 %Scores = [K2Score,LR,ME,HBP];  
